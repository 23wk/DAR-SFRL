    ...
    for eval_step in range(config['eval_epochs']):
        ...
        if eval_step % config['print_loss'] == 0:
            txt = 'eval step: {},\t linear train acc: {},\t linear train loss: {},\t' \
                  'eval linear acc: {},\t eval linear loss: {}'.format(
                eval_step,train_acc,train_loss,test_linear_acc,test_linear_loss)
            print(txt)
...
def main(config,logger):
    ...
    if config['eval']:
        ...
        print('config name: {},\t test linear acc is : {},\t'.format(config['experiment_name'],linear_acc))
        exit(0)

    pfe_train_dataset = PfeDB(config)
    ...
    logger.add_scalar('Total training time:',total_duration)
