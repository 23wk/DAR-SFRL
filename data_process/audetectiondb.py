...
class AUdetectionDB(data.Dataset):
    def __init__(self,config,phase):
        ...
        elif self.dataset == 'EmotiNet':
            if self.phase != 'train':
                self.img_root = config['val_root']
            self.data_list = self.get_list_emotiNet(self.list_root)
        else:
    ...
    def get_list_emotiNet(self,path):
        data_list = []
        with open(path,'r') as imf:
            for line in imf:
                arr = line.strip().split(',')
                img_path = arr[0]
                label_list = [int(arr[1]),int(arr[2]),int(arr[5]),int(arr[7]),int(arr[8]),int(arr[9]),int(arr[11])]
                label = np.array(label_list).astype(np.float32)
                data_list.append((img_path,label))
