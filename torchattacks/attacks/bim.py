...
class BIM(Attack):
    ...
    def __init__(self, model, linear=None, eps=8 / 255, alpha=2 / 255, steps=10):
    ...
    def forward(self, images, labels):
        ...
        for _ in range(self.steps):
            ...
            if self.linear is not None:
                clean, _ = self.get_logits(images)
                fea = clean[2].view(images.size()[0], -1)
                outputs = self.linear(fea)
            else:
                # clean, _ = self.get_logits(images)
                # outputs = clean[2].view(images.size()[0], -1)
                outputs = self.get_logits(images)
