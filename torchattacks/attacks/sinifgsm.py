...
class SINIFGSM(Attack):
    ...
    def __init__(self, model, linear=None, eps=8 / 255, alpha=2 / 255, steps=10, decay=1.0, m=5):
        ...
        for _ in range(self.steps):
            ...
            for i in torch.arange(self.m):
                ...
                if self.linear is not None:
                    clean, _ = self.get_logits(nes_images)
                    fea = clean[2].view(nes_images.size()[0], -1)
                    outputs = self.linear(fea)
                else:
                    clean, _ = self.get_logits(nes_images)
                    outputs = clean[2].view(nes_images.size()[0], -1)
                # Calculate loss
                if self.targeted:
