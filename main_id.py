...
def linear_eval(config,train_loader,val_loader,model,logger):
    ...
    linear_classifier = torch.nn.Sequential(
        torch.nn.BatchNorm1d(num_features=config['linear_dim']),
        torch.nn.Dropout(p=0.5), 
        torch.nn.Linear(in_features=config['linear_dim'],out_features=config['out_dim'])).cuda()
    optimizer = torch.optim.AdamW(linear_classifier.parameters(),lr=config['linear_lr'], weight_decay=config['wd'])
...
def main(config,logger):
    ...
    print('name: {},  test linear acc is : {}'.format(config['experiment_name'],linear_acc))
    exit(0)
